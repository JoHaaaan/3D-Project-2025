...
VertexBufferD3D11 vertexBuffer;
    ...
    ID3D11VertexShader* tessVS = ShaderLoader::CreateVertexShader(device, "TessellationVS.cso", &tessVSByteCode);
...
ID3D11SamplerState* shadowSampler = CreateShadowSampler(device);
 ...
 { {  1.0f, -1.0f, 0.0f }, {0,0,-1}, {1.0f, 1.0f} },
    ...
    gameObjects.emplace_back(cubeMesh);  // 0: Rotating cube
    gameObjects.emplace_back(cubeMesh);  // 1: Static cube
    gameObjects[1].SetWorldMatrix(XMMatrixTranslation(30.0f, 1.0f, 0.0f));
    gameObjects.emplace_back(pineAppleMesh);  // 2: Pineapple
    gameObjects[2].SetWorldMatrix(XMMatrixTranslation(0.0f, 0.0f, -14.0f));
    gameObjects.emplace_back(simpleCubeMesh);  // 3: SimpleCube 1 (reflective)
    gameObjects[3].SetWorldMatrix(XMMatrixTranslation(-2.0f, 2.0f, 0.0f));
    gameObjects.emplace_back(simpleCubeMesh);  // 4: SimpleCube 2
    gameObjects[4].SetWorldMatrix(XMMatrixTranslation(2.0f, 2.0f, 0.0f));
  gameObjects.emplace_back(simpleCubeMesh);  // 5: Ground plane
    gameObjects[5].SetWorldMatrix(XMMatrixScaling(5.0f, 0.2f, 5.0f) * XMMatrixTranslation(0.0f, -1.0f, 0.0f));
    gameObjects.emplace_back(sphereMesh);  // 6: Sphere
