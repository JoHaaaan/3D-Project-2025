...
HullShaderOutput HSMain(InputPatch<VertexShaderOutput, NUM_CONTROL_POINTS> ip, uint i : SV_OutputControlPointID, uint patchID : SV_PrimitiveID)
{
...
DomainShaderOutput DSMain(HS_CONSTANT_DATA_OUTPUT input, float3 barycentric : SV_DomainLocation, const OutputPatch<HullShaderOutput, NUM_CONTROL_POINTS> patch)
{
    ...
    output.worldPos = patch[0].worldPos * barycentric.x + 
